...
af1975b_1 <- function(x, p) p[1] + p[2]*exp(x) + p[3]*exp(-x)
af1975b_2 <- function(x, p) p[1] + p[2]*x + p[3]*x^2
af1975b_3 <- function(x, p) p[1] + p[2]*sin(0.5*pi*x) + p[3]*cos(0.5*pi*x) + p[4]*sin(pi*x)
af1975b_disp <- function(x, p) rep(p[1], length(x))
...
#MODEL_INFO = model_af1975b; DISTANCE = sq_diff; nSupp = 4; dsLower = -1; dsUpper = 1; MODEL_PAIR = MODEL_PAIR; WT_PAIR = NULL
#crit_type = "pair_fixed_true"; minWt = 0.0; MaxMinStdVals = NULL
#PSO_INFO = PSO_INFO; LBFGS_INFO = LBFGS_INFO; seed = 10; verbose = TRUE

MODEL_PAIR <- rbind(c(1, 2), c(1, 3))

PSOQN_af1975b <- DiscrimOD(MODEL_INFO = model_af1975b, DISTANCE = sq_diff, nSupp = 5,
                             dsLower = -1, dsUpper = 1, crit_type = "pair_multi_true",
                             MODEL_PAIR = MODEL_PAIR,
                             PSO_INFO = PSO_INFO, LBFGS_INFO = LBFGS_INFO,
                             seed = 10, verbose = TRUE)

PSOQN_af1975b$BESTDESIGN
#           dim_1     weight
...
# ----------------------------------------------------------------------------
# 6. Check equivalence theorem for each result
# ----------------------------------------------------------------------------

# Check PSO-QN result by the equivalence theorem
eqv_PSOQN <- equivalence(ngrid = 100, PSO_RESULT = PSOQN_af1975b,
                         MODEL_INFO = model_af1975b, DISTANCE = sq_diff,
                         dsLower = -1, dsUpper = 1, crit_type = "pair_multi_true",
                         MODEL_PAIR = MODEL_PAIR,
                         PSO_INFO = PSO_INFO, LBFGS_INFO = LBFGS_INFO)
...
points(PSOQN_af1975b$BESTDESIGN[,1], rep(0, nrow(PSOQN_af1975b$BESTDESIGN)), pch = 16)
