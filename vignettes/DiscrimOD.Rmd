...
The current status of **DiscrimOD** package is in the process of *alpha* test, i.e. it is open for development tean only. Before install **DiscrimOD** package, one need to install R software with version 3.4.0 and then the **devtool** package. Next, use `install_github` command in **devtool** to install **DiscrimOD** package from Ping-Yang Chen's github repository. The R codes are shown below. 
...
After installation, we use `library` to load the **devtool** package.
...
which is different from the work in \cite{atkinson1975optimal}.
...
The first step is to create the model list required by **DiscrimOD** package.  The guideline in setting the model list can be found by calling the `emptyModelList` function.
```{r call_empty, cache=T}
emptyModelList(N_model = 2)
```
The model list consists of several sub-lists and its length is equal to the number fo competing models. In the first sub-list, we provide the information of the true model with two components, `model` for storing the user-defined R function (here, `m1` is assigned) and `para` is the vector fo nominal values.
...
For $T$-optimal design, we specify the distance measure, squared difference between two models, in R function.  The distance function also has two inputs. The first input must be the mean vector of the true model and the second input must be the mean vector of the rival model. Although the order of inputs seems to be irrelevant for the squared difference, but this input order will be important if one consider the KL-divergence.
...
The algorithm settings are defined through `getAlgInfo()` function.  Based on our experience, belows are the most influential tuning parameters of the PSO-QN algorithm.
They are:

* `nSwarm`: The size of the particle swarm. Typically, we set 16 or 32 partciels.
* `maxIter`: The number of maximal iterations. 200 iterations should be enough for the example here.
* `LBFGS_RETRY`: The maximal times of trails of L-BFGS algorithm. To avoid L-BFGS algorihtm collapses due to a poor initial vaector, we may want to try 3 to 5 times of the algorithm.
* `GRAD_EPS`: The tolerance for convergence test on the gradient norm. The default is $10^{-10}$.
...
we need to input the type of discrimination criterion (for pairwise discrimination it is `"pair_fixed_true"`), the number of suppoer points `nSupp` and the lower nad upper bound of the design space, `dsLower` and `dsUpper`, respecitvely.
...
```{r AF1975b_EQUV_PAIR_12, cache=T, fig.align='center', fig.height = 4, fig.width = 4, fig.cap='Directional derivative function of T-optimal design in the case DISC_12.'}
...
```{r AF1975b_EQUV_PAIR_13, cache=T, fig.align='center', fig.height = 4, fig.width = 4, fig.cap='Directional derivative function of T-optimal design in the case DISC_13.'}
...
```{r AF1975b_EQUV_MAXMIN, cache=T, fig.align='center', fig.height = 4, fig.width = 4, fig.cap='Directional derivative function of max-min T-optimal design in the case DISC_MAXMIN.'}
...
```{r AF1975b_EQUV_PAIR_12_CPP, cache=T, fig.align='center', fig.height = 4, fig.width = 4, fig.cap='Directional derivative function of T-optimal design in the case DISC_CPP_12.'}
...
```{r AF1975b_EQUV_PAIR_13_CPP, cache=T, fig.align='center', fig.height = 4, fig.width = 4, fig.cap='Directional derivative function of T-optimal design in the case DISC_CPP_13.'}
...
```{r AF1975b_EQUV_MAXMIN_CPP, cache=T, fig.align='center', fig.height = 4, fig.width = 4, fig.cap='Directional derivative function of max-min T-optimal design in the case DISC_CPP_MAXMIN.'}
